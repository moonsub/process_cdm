...
def test_doctorAI(
	...
	for batchIndex in range(n_batches):
		...
		for i in range(codeResults.shape[1]):
			...
			thisY = tempY[i][1:]
			thisX = tempX[i][:]
			print('------------------------------------------------- %d  번째 환자 (%d 번의 방문) -------------------------------------------------' % (i, len(thisX)))

			
			for timeIndex in range(lengths[i]):
				print('---------------------------------------------------------- %d  번째 방문 ----------------------------------------------------------' % (timeIndex + 1))

				if timeIndex < lengths[i] - 1:
				...
				predict = zip(*heapq.nlargest(30, enumerate(output), key=operator.itemgetter(1)))[0]

				if timeIndex < lengths[i] - 1:
					y_count = len(set(y))
					inter_set = set(y).intersection(set(predict))
					inter_count = len(inter_set)
				...
				print('intersection : %s' % (inter_set))
				print('target_count : %d/%d' % (inter_count, y_count))
...
if __name__ == '__main__':
	...
	'''
	test_doctorAI(
		modelFile='/Users/xiilab/Desktop/dev/git/doctorai/cdm_data5/model/model.996.npz',
		seqFile='/Users/xiilab/Desktop/dev/git/doctorai/cdm_data5/data.train.train', 
		labelFile='/Users/xiilab/Desktop/dev/git/doctorai/cdm_data5/data.train.train', 
